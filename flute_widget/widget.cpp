        ...
        for (int i = 0; i < newList.size(); i++){
           ...
           QString noteString = (QString::fromStdString(newList.at(i).toStdString())).chopped(2);
            ...
            for (int x =0; x < (int)thisVector.size(); x++){
                ...
                QFileInfo check_file(":/music_resources/flute_" + (QString::fromStdString(thisVector.at(x).returnNoteName()) + QString::number(valueOfOctave.toInt())) + "_1_forte_normal.wav");
...
void Widget::on_pushButton_6_clicked(){
    QPixmap pm(":/fluteResources/flute_hint.jpg");
    QMessageBox helpBox;
